...
import os
import random
# import timepython cifar_train.py --gpu 0 --imb_type exp --imb_factor 0.01 --loss_type CE --train_rule None
import time 
import warnings
import sys
import numpy as np
...
from mymodels.resnetwithkan import *
import logging
...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet32',
                    ...
                    help='model architecture: ' +
                        ' | '.join(model_names) +
                        ' (default: resnet32)')
parser.add_argument('--loss_type', default="LA", type=str, help='loss type')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.kan:
        model = models.__dict__["resnet32withkan"](num_classes=num_classes)
        print('using kan for training')
    else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        ##############
            ...
            base_probs = torch.tensor([n / sum(cls_num_list) for n in cls_num_list])
            print("base_probs is:",base_probs)
            base_probs = base_probs.cuda(args.gpu) 
            use_la_loss = True  
            criterion = LALoss(use_la_loss, base_probs, 1.02).cuda(args.gpu)
        else:
...
def train(train_loader, model, criterion, optimizer, epoch, args, log, tf_writer):
    ...
    # logging.info(f"Epoch: {epoch}, Train Loss: {losses.avg}, Train Acc: {top1.avg},Train Acc5: {top5.avg}")
...
######################
        ...
        res = {str(i):x for i, x in enumerate(cls_acc)}
        logging.info(f" Train Acc: {res}")
